        ...
        if ret == True:
            ...
            if possible_plates is not None:
                for i, p in enumerate(possible_plates):
                    ...
                    recognized_plate = pytesseract.image_to_string(p,config='tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ')
                    recognized_plate = recognized_plate.replace("\n", "")                   
                    #print(len(recognized_plate))
                    if len(recognized_plate)==8:                    
                        if recognized_plate[0] in ("1","2","3","4","5","6","7","8","9","0"):
